...
public class AppUsageStatisticsActivity extends AppCompatActivity {
    ...
    private static final String TAG = AppUsageStatisticsActivity.class.getSimpleName();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Editar: Eliminar cuando se convierta en Fragmento
        ...
        // Editar: Eliminar
        // Llamar al metodo para obtener las estadísticas (por ejemplo, por mes)
        List<UsageStats> stats = getUsageStatistics(UsageStatsManager.INTERVAL_MONTHLY);

        // Mostrar en Logcat cuántos registros se obtuvieron
        if (stats != null) {
            Log.i(TAG, "Número de apps con estadísticas: " + stats.size());
            for (UsageStats stat : stats) {
                Log.d("USAGE", "Paquete: " + stat.getPackageName());
                Log.d("USAGE", "Último uso: " + stat.getLastTimeUsed());
                Log.d("USAGE", "Tiempo total en primer plano: " + stat.getTotalTimeInForeground() + " ms");
            }
        }
    }
    ...
    public List<UsageStats> getUsageStatistics(int intervalType) {
       ...
       */
        ...
        if (queryUsageStats.size() == 0) {
            ...
            Toast.makeText(this, getString(R.string.explanation_access_to_appusage_is_not_enabled), Toast.LENGTH_LONG).show();
